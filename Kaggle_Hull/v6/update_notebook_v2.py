...
# Define the new inference block
inference_block = """# -----------------------------------------------------------------------------------------
...
# Note: Previous replace might have slightly different spacing, so we use flexible regex.
...
if new_source == source_str:
    ...
    # Debug: print a chunk of the source to see what we are missing
    print("--- snippet ---")
    start_search = source_str.find("# 6. INFERENCE LOOP")
    if start_search != -1:
        print(source_str[start_search:start_search+500])
    else:
